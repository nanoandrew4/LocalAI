...
class BackendServicer(backend_pb2_grpc.BackendServicer):
    ...
    def LoadModel(self, request, context):
        ...
        # Enable performance optimizations from the streaming fork
        # This provides ~6x speedup via torch.compile and fast codebook generation
        try:
            decode_window_frames = self.options.get("decode_window_frames", 80)
            use_compile = self.options.get("use_compile", True)
            use_fast_codebook = self.options.get("use_fast_codebook", True)

            # Set TensorFloat32 for better performance on Ampere+ GPUs
            if self.device == "cuda":
            ...
            self.model.enable_streaming_optimizations(
                ...
                compile_mode="reduce-overhead",  # Faster warmup than max-autotune
                use_fast_codebook=use_fast_codebook,
                compile_codebook_predictor=True,
                compile_talker=True,
            )
            print(
                f"Enabled streaming optimizations: decode_window_frames={decode_window_frames}, compile={use_compile}",
                file=sys.stderr,
            )
        except Exception as e:
            print(
                f"Warning: Could not enable streaming optimizations: {e}",
                file=sys.stderr,
            )
            # Continue without optimizations - model will still work
